...
t = [0:ts:size(archivo.tom)/fs - ts];   % Creación del vector tiempo en base a ts y los datos del archivo
...
potencia = sum(abs(x(limiteInf:limiteSup)).^2) / (limiteSup - limiteInf)    % Cálculo de la potencia

tCuerda = t(limiteInf:limiteSup);   % Creación del vector de tiempo de la primera cuerda (desde que comienza hasta 10ms después)
xCuerda = x(limiteInf:limiteSup);   % Creación del vector de datos de la primera cuerda (desde que comienza hasta 10ms después)

% Gráfico de la primera cuerda (desde que comienza hasta 10ms después)
figure
plot(tCuerda, xCuerda)

% Cálculo de la transformada de Fourier. (Código obtenido del ejemplo 4.0 entregado por el profesor en canvas)
